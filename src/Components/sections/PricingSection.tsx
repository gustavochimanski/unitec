...
const PricingSection = ({
  ...
  const planWhatsAppUrl = (plan: Plan) =>
    comingSoon
      ...
      : getWhatsAppUrl(
          `Olá! Quero o plano ${plan.name} do Mensura E-commerce, com o 1º mês grátis. Podemos conversar?`
        )

  return (
    <section id={sectionId} className="py-16 md:py-24 bg-white font-sans">
      <div className="container mx-auto px-4 md:px-6">
        <h2 className="text-4xl md:text-5xl font-bold text-center text-indigo-800 mb-4">{title}</h2>
        <p className="text-lg text-center text-gray-600 max-w-3xl mx-auto mb-12">{subtitle}</p>
          ...
          {plans.map((plan) => (
            <Card
              ...
              className={`relative flex flex-col ${
                plan.featured
                  ? "border-2 border-indigo-800 shadow-xl scale-[1.02]"
              ...
              <CardContent className="flex-grow">
                <div className="text-center mb-6">
                  {plan.price !== null ? (
                    <div>
                      {plan.firstMonthFree && (
                        <p className="text-emerald-600 font-semibold text-sm mb-1">1º mês grátis</p>
                      )}
                      {plan.originalPrice && (
                      ...
                      {plan.firstMonthFree && (
                        <p className="text-xs text-gray-500 mt-1">a partir do 2º mês</p>
                      )}
                    </div>
                  ) : (
                    <div>
